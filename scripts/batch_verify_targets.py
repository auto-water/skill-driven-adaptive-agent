...
默认向子进程设置 INIT_LOG_CATEGORY=<batch-root 目录名>，使日志落在 codes/logs/<main 父目录名>/<类别>/<前缀>-<slug>/，
...
推荐与批量测试一起在 shell 或 --env 中设置的 main.py 变量（详见对应 impl 目录下的 main.py）：
  ...
  OPENROUTER_CHAT_TIMEOUT_SEC  可选；设置后覆盖 SDK 默认读超时（约 600s），并默认 max_retries=0；见 main.py
  OPENROUTER_MAX_RETRIES       与上一项联用；未设 OPENROUTER_CHAT_TIMEOUT_SEC 时仍默认 2

本脚本 CLI：
  --main-timeout-sec      单靶场子进程最长等待（默认 900=15min；0 为不限制）
...
子进程若触发 --main-timeout-sec 超时：整靶场记为失败，不复制 metrics（避免会话目录内历史 JSON 误判为本次结果）。
metrics 另含 main_exit_reason、limit_hit_*（token / 墙钟 / 单次请求超时 / 轮数）字段便于分析中断原因。
"""
...
# main.py 单靶场子进程默认最长等待（秒）；0 表示不限制；默认 15 分钟
_DEFAULT_MAIN_TIMEOUT_SEC = 600


def _sanitize_init_log_category(raw: str) -> str:
    """与 main.py 一致，用于拼接 codes/logs/<main 父目录名>/<category>/。"""
    s = raw.strip()
    if not s:
        return ""
...
def _session_log_root(main_py: Path, init_log_category: Optional[str] = None) -> Path:
    """codes/logs/<main.py 所在目录名>[/INIT_LOG_CATEGORY]（与对应 main.py 的 SESSION_LOG_ROOT 一致）。"""
    impl = main_py.resolve().parent.name
    base = main_py.resolve().parent.parent / "logs" / impl
    ...
    if session_dir.is_dir():
        for p in sorted(
            ...
            if p.is_file() and is_fresh(p):
                return p
    # min_mtime>0 时禁止回退到「仅存在即可」：否则会误用历史 metrics（如子进程超时退出后仍复制旧文件）。
    if min_mtime > 0:
        return None
    if primary.is_file():
        return primary
...
def run_one_case(
    *,
    ...
    try:
        ...
        print(
            "[错误] 本次已超时终止，不复制会话目录 metrics（避免沿用历史成功结果）。",
            flush=True,
        )
        return 1
    ...
    if src is None or not src.is_file():
        ...
        seg = main_py.resolve().parent.name
        sub = f"{seg}/{cat}/{label}" if cat else f"{seg}/{label}"
        print(
...
def main() -> int:
    ap = argparse.ArgumentParser(description="批量运行指定的 main.py 并汇总 metrics")
    ap.add_argument(
        "--batch-root",
        type=Path,
        required=True,
    ...
    ap.add_argument(
        "--flat-init-logs",
        action="store_true",
        help="不传 INIT_LOG_CATEGORY，会话仍写入 codes/logs/<main 父目录名>/<前缀>-<slug>/（与旧行为一致）",
    )
    ap.add_argument(
        "--main-timeout-sec",
        type=int,
